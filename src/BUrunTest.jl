...
function prediction(m::LearningPi.SampleTransformer, x, instance, _::LearningPi.learningSampleTransformer, dt::LearningPi.abstract_deviation)
        μσ = m.model[1](x, x.ndata.x)
        @views μ = μσ[1:Int64(end / 2), :]
        @views o = m.model[2](μ)[1:prod(x.gdata.u)]
        o = reshape(o, cpu(x.gdata.u)...) + LearningPi.deviationFrom(x,dt)
        return o
end
...
function fill_tables!(ds,dataset,factory,lt,fmt,nn1,crs,golds,lrs,predsK,preds,predsK_time,preds_time,LRk_time,LR_time,CR_time,FE_time,K)
	for instPath in ds[dataset]             
		...
		t0=time()
		pred=prediction(nn1, ϕ |> gpu,inst,lt, LearningPi.cr_deviation()) |> cpu	
...
function main(args)
	...
	parsed_args = parse_args(args, s)
	    ...
	    lt = get_from_LearningPi(parsed_args["learningType"][1])
	    factory = get_from_LearningPi(parsed_args["factory"][1])
    ...
    for (seed,sub_string) in [(seeds[i],model_folders[i]) for i in eachindex(model_folders)]
        path = "/users/demelas/learning_pi_slim.jl/launchers/run/HL_" * sub_string * "/"
        ...
        @load model_path nn
        nn = nn |> gpu
        nn1=LearningPi.SampleTransformer(nn,Random.MersenneTwister(parse(Int64,seed)))
        ...
        global	m_gap_CR_train += gap_CR_train
        global	m_gap_LR_train += gap_LR_train
        global	m_gap_pred_train += gap_pred_train
        global	m_gap_max_pred_train += gap_max_pred_train
        
        global	m_gap_CR_val += gap_CR_val
        global	m_gap_LR_val += gap_LR_val
        global	m_gap_pred_val += gap_pred_val
        global	m_gap_max_pred_val += gap_max_pred_val
        
        global	m_gap_CR_test += gap_CR_test
        global	m_gap_LR_test += gap_LR_test
        global	m_gap_pred_test += gap_pred_test
        global	m_gap_max_pred_test += gap_max_pred_test
        
        
        global	m_perc_CR_train += perc_CR_train
        global	m_perc_LR_train += perc_LR_train
        global	m_perc_pred_train += perc_pred_train
        global	m_perc_max_pred_train += perc_max_pred_train
        
        global	m_perc_CR_val += perc_CR_val
        global	m_perc_LR_val += perc_LR_val
        global	m_perc_pred_val += perc_pred_val
        global	m_perc_max_pred_val += perc_max_pred_val
        
        global	m_perc_CR_test += perc_CR_test
        global	m_perc_LR_test += perc_LR_test
        global	m_perc_pred_test += perc_pred_test
        global	m_perc_max_pred_test += perc_max_pred_test
    end	
